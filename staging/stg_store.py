...
table_id = 'stg_store' # Modify the necessary table name: stg_customer, stg_city etc.
...
with base as (
  select *
  from `capstone-project-495705.pagila_productionpublic.store` --Your table path
  )

  , final as (
    select
        store_id as store_address_id
        , manager_staff_id
        , address_id
